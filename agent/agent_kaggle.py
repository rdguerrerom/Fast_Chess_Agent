...
class OpeningBook:
    ...
    def get_book_move(self, position_fen: str) -> Tuple[int, int]:
        """
        Get a move from the opening book, with fallback strategy.
        
        If no book move is found, returns a default move to ensure a valid move is always returned.
        """
        if len(self.book) >= self.max_positions:
            # Fallback move: move a knight or a pawn if no book move exists
            fallback_moves = [
                ((6, 0), (5, 2)),  # Knight f3
                ((6, 7), (5, 5)),  # Knight f6
                ((4, 1), (4, 3)),  # Pawn e4
        ...
        # If no moves found in book, use fallback moves
        fallback_moves = [
            ((6, 0), (5, 2)),  # Knight f3
            ((6, 7), (5, 5)),  # Knight f6
            ((4, 1), (4, 3)),  # Pawn e4
...
class MiddlegameBook:
    ...
    def get_strategic_recommendation(
        ...
        """
        Get sophisticated strategic recommendations with fallback strategy
        ...
        if not matching_strategies:
            fallback_move = self._generate_fallback_move(fen)
            return {
                "strategy_name": "Flexible Development",
                "recommended_move": fallback_move,
                "move_description": "Improve piece coordination and central control",
        ...
        best_strategy = max(
            ...
            key=lambda x: max(
                ...
                if x[1].get("recommended_moves")  # Ensure moves exist
            ) if x[1].get("recommended_moves") else 0  # Default to 0 if no moves
        )

        # Handle case where no recommended moves exist
        recommended_moves = best_strategy[1].get("recommended_moves", [])
        
        # If no recommended moves, generate a fallback move
        if not recommended_moves:
            fallback_move = self._generate_fallback_move(fen)
            return {
                "strategy_name": best_strategy[0],
                "recommended_move": fallback_move,
                "move_description": "Flexible development due to lack of specific strategy",
                "position_evaluation": position_eval,
            }
    ...
    def _generate_fallback_move(self, fen: str) -> Tuple[int, int]:
        """
        Generate a sensible fallback move when no specific strategy is found

        Args:
            fen (str): Current board position

        Returns:
            Tuple[int, int]: A recommended move coordinates
        """
        # Fallback moves prioritizing piece development and central control
        fallback_moves = [
            ((6, 0), (5, 2)),  # Knight to f3
            ((6, 7), (5, 5)),  # Knight to f6
            ((4, 1), (4, 3)),  # Pawn to e4
            ((3, 1), (3, 3)),  # Pawn to d4
            ((1, 0), (2, 2)),  # Knight to c3
            ((5, 0), (3, 2)),  # Bishop to d2
        ]
        
        # Randomize fallback moves to avoid predictability
        return random.choice(fallback_moves)
...
class EndgameBook:
    ...
    def get_endgame_recommendation(
        ...
        # If no matching strategies, provide a default flexible strategy
        if not matching_strategies:
            return {
                "strategy": "Flexible endgame play",
                "strategy_description": "Maintain piece activity and control key squares",
                "recommended_move": ((1, 3), (1, 2)),  # Default conservative king move
                "move_tactics": "Maintain position, limit exposure",
                "key_objectives": [
                    "Maintain piece activity",
                    "Control key squares",
                    "Minimize opponent's counterplay",
                ],
                "winning_potential": 0.5,
            }
        ...
        # If no recommended moves, use a default conservative move
        if not recommended_moves:
            return {
                "strategy": best_strategy[0],
                "strategy_description": recommended_strategy.get("description", "Conservative play"),
                "recommended_move": ((1, 3), (1, 2)),  # Default conservative king move
                "move_tactics": "Maintain position, limit exposure",
                "winning_potential": position_eval.get_winning_potential(),
            }

        # Select recommended move with complex scoring
        recommended_move = max(
            recommended_moves,
            key=lambda move: (
                move["score"] * 0.6
...
class ChessPosition:
    def __init__(self):
        # Correctly initialize both white and black pawns
        self.piece_positions = {
            "P": [0x000000000000FF00, 0x00FF000000000000],  # White and black pawns
            "R": [0x0000000000000081, 0x8100000000000000],  # Rooks
            "N": [0x0000000000000042, 0x4200000000000000],  # Knights
            "B": [0x0000000000000024, 0x2400000000000000],  # Bishops
            "Q": [0x0000000000000008, 0x0800000000000000],  # Queens
            "K": [0x0000000000000010, 0x1000000000000000],  # Kings
        }
        self.white_pieces = sum(bb[0] for bb in self.piece_positions.values())
        self.black_pieces = sum(bb[1] for bb in self.piece_positions.values())
...
class ChessAgent:
    def __init__(self, max_depth=10, max_time=5.0):
    ...
    def _convert_move_to_uci(self, move: Tuple[int, int]) -> str:
        """
        Convert a move from (from_square, to_square) to UCI notation.
        """
        files = "abcdefgh"
        ranks = "12345678"
        ...
        # Handle promotion (assuming promotion to queen)
        promotion_piece = ''
        moving_piece = self._get_piece_at(self.current_position, from_square)
        if moving_piece == 'P' and (to_square // 8 == 0 or to_square // 8 == 7):
            promotion_piece = 'q' if self.current_position.white_to_move else 'Q'

        return f"{from_file}{from_rank}{to_file}{to_rank}{promotion_piece}"

    def get_best_move(self, position: ChessPosition) -> Optional[str]:
        """
        Integrated move selection strategy with UCI-style move output
        ...
        if game_phase == "opening" and len(self.position_history) < 15:
            ...
            if book_move:
                # Convert coordinate tuples to square indices
                from_square, to_square = book_move
                if isinstance(from_square, tuple):
                    from_square = self._coordinates_to_square(from_square)
                if isinstance(to_square, tuple):
                    to_square = self._coordinates_to_square(to_square)
                move = (from_square, to_square)
                return self._convert_move_to_uci(move)
        ...
        if game_phase == "middlegame":
            ...
            if recommended_move:
                # Convert coordinate tuples to square indices
                from_square, to_square = recommended_move
                if isinstance(from_square, tuple):
                    from_square = self._coordinates_to_square(from_square)
                if isinstance(to_square, tuple):
                    to_square = self._coordinates_to_square(to_square)
        ...
        if not moves:
            ...
            friendly_occupancy = self._get_friendly_occupancy(position)
            enemy_occupancy = self._get_enemy_occupancy(position)
            empty_squares = ~(friendly_occupancy | enemy_occupancy) & 0xFFFFFFFFFFFFFFFF
        ...
        return (
            self._convert_move_to_uci(best_move_coords)
            if best_move_coords
            else None
        )
    ...
    def _search_best_move(
        ...
        """
        Iterative deepening search with alpha-beta pruning and fallback move selection.
        ...
        # Fallback move selection if no optimal move found
        if best_move is None:
            best_move = self._select_least_worse_move(position, moves)
        
        return best_move

    def _select_least_worse_move(
        self, position: ChessPosition, moves: List[Tuple[int, int]]
    ) -> Optional[Tuple[int, int]]:
        """
        Select the least disadvantageous move when no optimal move is found.
        
        Strategy:
        1. Prioritize moves that minimize piece loss
        2. Avoid moves that result in immediate capture
        3. Prefer non-capture moves
        4. If all else fails, return the first available move
        """
        if not moves:
            return None
        
        # Moves sorted by minimizing potential piece loss
        scored_moves = []
        for move in moves:
            score = 0
            piece_at_from = self._get_piece_at(position, move[0])
            piece_at_to = self._get_piece_at(position, move[1])
            
            # Piece value difference as primary scoring mechanism
            if piece_at_to:
                piece_from_value = self._get_piece_value(piece_at_from)
                piece_to_value = self._get_piece_value(piece_at_to)
                score = piece_from_value - piece_to_value
            
            # Slight penalty for capture moves
            if self._is_capture(position, move):
                score -= 0.5
            
            scored_moves.append((move, score))
        
        # Sort moves by score, minimizing piece loss
        scored_moves.sort(key=lambda x: x[1])
        
        # Return the move with the least negative impact
        return scored_moves[0][0] if scored_moves else moves[0]
    ...
    def _get_piece_at(self, position: ChessPosition, square: int) -> Optional[str]:
        """
        Determine the piece at a given square.
        """
        for piece, (white_bb, black_bb) in position.piece_positions.items():
    ...
    def _generate_legal_moves(self, position: ChessPosition) -> List[Tuple[int, int]]:
        ...
        and implementing a piece exchange evaluation strategy.
        """
        moves = self._generate_moves(position)
        legal_moves = []
        
        # If no moves are available, return None or empty list
        if not moves:
            return legal_moves

        # Categorize moves based on exchange value
        move_exchanges = []
        for move in moves:
            if not self._move_leaves_king_in_check(position, move):
                # Evaluate piece exchange value
                exchange_value = self._evaluate_piece_exchange(position, move)
                legal_moves.append(move)
                move_exchanges.append((move, exchange_value))

        # If no legal moves exist, return an empty list
        if not move_exchanges:
            return legal_moves

        # Sort moves by exchange value (least loss is preferred)
        move_exchanges.sort(key=lambda x: x[1], reverse=True)
        
        # Return the move with the least material loss
        return [move_exchanges[0][0]]

    def _evaluate_piece_exchange(self, position: ChessPosition, move: Tuple[int, int]) -> float:
        """
        Evaluate the piece exchange value of a move.
        
        Returns a score representing the material gain/loss:
        - Positive values indicate a favorable exchange
        - Negative values indicate an unfavorable exchange
        """
        from_square, to_square = move[0], move[1]
        
        # Simulate the move to get the new board position
        new_position = self._simulate_move(position, move)
        
        # Get pieces involved in the exchange
        moving_piece = self._get_piece_at(position, from_square)
        captured_piece = self._get_piece_at(position, to_square)
        
        # Piece value dictionary (standard chess piece values)
        piece_values = {
            'P': 1.0,   # Pawn
            'N': 3.0,   # Knight
            'B': 3.0,   # Bishop
            'R': 5.0,   # Rook
            'Q': 9.0,   # Queen
            'K': 0.0    # King (infinite value, should not be exchanged)
        }
        
        # If no piece is captured, return a neutral value
        if not captured_piece:
            return 0.0
        
        # Calculate exchange value
        moving_piece_value = piece_values.get(moving_piece, 0.0)
        captured_piece_value = piece_values.get(captured_piece, 0.0)
        
        # Favor exchanges that result in material gain
        exchange_value = captured_piece_value - moving_piece_value
...
def chess_bot(obs):
    """
    Determines the best move for the current chess position.

    Args:
        obs: An object with a 'board' attribute representing the current board state in FEN format.

    Returns:
        A string representing the chosen move in UCI notation (e.g., 'e2e4').
    """
    # Initialize your chess agent
    agent = ChessAgent()

    # Retrieve the current board state in FEN format
    current_fen = obs.board

    # Convert FEN to ChessPosition
    position = FENTranslator.fen_to_bitboard(current_fen)

    # Determine the best move using your agent
    best_move = agent.get_best_move(position)
