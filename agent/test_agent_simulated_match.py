...
class ChessAgentPerformanceAssessment:
    ...
    def _get_initial_position(self):
        ...
        return FENTranslator.fen_to_bitboard(
            "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
        )

    def _evaluate_move_quality(
        self, old_position: ChessPosition, new_position: ChessPosition
    ) -> float:
        """
    ...
    def _check_piece_development(
        self, old_pos: ChessPosition, new_pos: ChessPosition
    ) -> float:
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        num_total_moves = sum(
            len(moves) for moves in report["phase_performance"].values()
        )

        report["overall_metrics"] = {
            "avg_moves_per_game": total_moves / len(self.game_simulations),
            "avg_move_quality": total_move_quality / num_total_moves
            if num_total_moves > 0
            else 0,
            "avg_calculation_speed": 1 / (total_calculation_time / num_total_moves)
            if num_total_moves > 0
            else 0,
        }
...
def get_move_made(old_position: ChessPosition, new_position: ChessPosition) -> str:
    ...
    for piece in old_position.piece_positions:
        ...
        moved_from = (old_white_bb if old_position.white_to_move else old_black_bb) & ~(
            new_white_bb if old_position.white_to_move else new_black_bb
        )
        moved_to = (new_white_bb if old_position.white_to_move else new_black_bb) & ~(
            old_white_bb if old_position.white_to_move else old_black_bb
        )
...
def main():
    ...
    print(
        f"Average Moves per Game: {performance_report['overall_metrics']['avg_moves_per_game']:.2f}"
    )
    print(
        f"Average Move Quality: {performance_report['overall_metrics']['avg_move_quality']:.3f}"
    )
    print(
        f"Average Calculation Speed Score: {performance_report['overall_metrics']['avg_calculation_speed']:.3f}"
    )
    ...
    for phase, moves in performance_report["phase_performance"].items():
        if moves:
            ...
            print(
                f"  Avg Move Quality: {sum(m['move_quality'] for m in moves) / len(moves):.3f}"
            )
            print(
                f"  Avg Tactical Complexity: {sum(m['tactical_complexity'] for m in moves) / len(moves):.3f}"
            )
